...
$passenger_revenue = mysqli_fetch_assoc(mysqli_query($conn, "SELECT SUM(total_price) AS total FROM bookings"))['total'] ?? 0;
$cargo_revenue = mysqli_fetch_assoc(mysqli_query($conn, "SELECT SUM(total_price) AS total FROM cargo_bookings"))['total'] ?? 0;
