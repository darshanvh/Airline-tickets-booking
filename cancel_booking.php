...
// Get total records count for future bookings only
$total_query = "SELECT COUNT(*) AS total 
                FROM bookings b 
                JOIN flights f ON b.flight_id = f.id 
                WHERE b.user_id = ? 
                AND (CONCAT(f.date, ' ', f.departure_time) >= NOW() OR b.status = 'Pending')";
...
// Modify the query to handle all booking formats
// Modify SQL query
$query = "SELECT 
            b.id AS booking_id, 
            f.flight_name, 
            ...
            f.date, 
            b.seat_type, 
            COALESCE(b.seat_numbers, b.seat_number) as seat_numbers, 
            COALESCE(b.total_amount, b.amount, b.total_price) as total_price, 
            b.status,
            b.reference_id,
            b.name,
            b.aadhar_number,
          ...
          WHERE b.user_id = ? 
          AND (CONCAT(f.date, ' ', f.departure_time) >= NOW() OR b.status = 'Pending')
          ORDER BY f.date DESC, f.departure_time DESC, b.id DESC 
          LIMIT ?, ?";
...
<body>
                        ...
                        <tr>
                            ...
                            <td><?php 
                                if (!empty($row['seat_numbers'])) {
                                    echo htmlspecialchars($row['seat_numbers']);
                                } elseif (!empty($row['seat_number'])) {
                                    echo htmlspecialchars($row['seat_number']);
                                }
                            ?></td>
                            ...
                            <td>
                                <?php if ($row['status'] == 'Booked' || $row['status'] == 'Pending'): ?>
                                    <button class="cancel-button" onclick="openModal(<?php echo $row['booking_id']; ?>)">
    ...
    <div id="myModal" class="modal">
        <div class="modal-content">
            ...
            <form id="cancelForm" method="POST" action="processs_cancel.php">
